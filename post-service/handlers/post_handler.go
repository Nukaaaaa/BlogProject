...
func GetUserFromUserService(c *gin.Context) {
	userID := c.Param("id")
	requestID := c.MustGet("RequestID").(string) // Извлекаем RequestId из контекста
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Error calling user-service"})
		return
	}

	// Логируем успешный ответ от user-service
	if resp.StatusCode() == http.StatusOK {
		log.Printf("[RequestID: %s] Successfully fetched user data: %+v", requestID, resp.Result())
	} else {
		log.Printf("[RequestID: %s] Error fetching user data from user-service: %d", requestID, resp.StatusCode())
	}
